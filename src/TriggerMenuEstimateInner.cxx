...
void TriggerMenuEstimate::Tile()
{
  for(int i=0;i!=(int)RoI_pt.size();i++){
    if(RoI_sou.at(i)!=1){continue;}
    bool isTile = false;
    if(TileOKS(RoI_side.at(i), RoI_sec.at(i), RoI_roi.at(i)) && RoI_roi.at(i)<=39){
...
void TriggerMenuEstimate::EIFI()
{
  ...
  for(int i=0;i!=(int)RoI_pt.size();i++){
    ...
    if(EIFIOKS(RoI_side.at(i),RoI_sec.at(i),RoI_roi.at(i))){
      if(EIFIRoI(RoI_side.at(i),RoI_sec.at(i),RoI_roi.at(i))){
        float DR = 100;
        int roinum = -100;
